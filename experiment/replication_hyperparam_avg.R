...
print(paste0("Averaging over pip values", seq(p_start, p_stop, p_by)))
...
# 
# # hyperparameter grid
# p_incl_grid = seq(p_start, p_stop, p_by)
# v_noise = 1
# v_slab = 3
# 
# n_sim = 50
...
# 
# 
# filename = paste0(output_path, "discrete_independent.RDS")
...
# })
# 
# filename = paste0(output_path, "no_covariate.RDS")
# saveRDS(metrics, file = filename)
# 
...
#   })
# 
#   filename = paste0(output_path, "discrete_dependent_", "p=", p, ".RDS")
#   saveRDS(metrics, file = filename)
# }
...
progressr::with_progress({
  ...
  incl_prob_dt = 
    foreach(sim_idx = 1:n_sim, .combine = rbind, .errorhandling = 'remove') %dorng% {
      
      prog(sprintf("Simulation %g, %s", p, sim_idx, Sys.time()))
      
      # simulate data
      for(i in 1:n) {
        X[i, ] = MASS::mvrnorm(1, rep(0, p+1), solve(Prec_cont(Z[i])))
      }
      data_mat = X
      ...
      incl_probs = rbind(
        epwpl::incl_prob_123(ep_vopt_result),
        epwpl::incl_prob_123(varbvs_vopt_result),
        epwpl::incl_prob_123_combo(ep_vopt_result, varbvs_vopt_result, .25),
        epwpl::incl_prob_123_combo(ep_vopt_result, varbvs_vopt_result, .5),
        epwpl::incl_prob_123_combo(ep_vopt_result, varbvs_vopt_result, .75)
      )
      incl_probs[, sim_idx := sim_idx]
      
      # filename = paste0(output_path, "continuous/",
      #                   "ep_",
      #                   Sys.Date(),
      #                   "_sim=", sim_idx,
...
filename = paste0(output_path, "continuous.RDS")
